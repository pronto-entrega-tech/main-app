...
const lanIp =
  Constants.expoConfig?.extra?.lanIp ?? process.env.NEXT_PUBLIC_LAN_IP;
...
const DEV_API = `http${USE_HTTPS ? "s" : ""}://${lanIp}:3000`;
...
const STATIC = lanIp ? DEV_STATIC : PROD_STATIC;

const API = lanIp ? DEV_API : PROD_API;

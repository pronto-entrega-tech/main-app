...
function MyProvider(props: any) {
  ...
  const [shoppingList, setShoppingList] = React.useState<
    Map<string, { quantity: number; item: prodModel }>
  >(new Map());
  const [notify, setNotify] = React.useState<Map<string, prodModel>>(new Map());
  const [favorites, setFavorites] = React.useState<Map<string, prodModel>>(
    new Map()
  );
  ...
  React.useEffect(() => {
    getIsGuest().then((isGuest) => setIsGuest(isGuest));

    getActiveMarketKey().then((key) => setActiveMarketKey(key));
  ...
  const onPressAdd = (item: prodModel) => {
    ...
    let value: number | undefined = shoppingList.has(item.prod_id)
      ? shoppingList.get(item.prod_id)?.quantity
      : 0;
    if (typeof value == 'undefined')
      return Alert.alert('Erro ao adicionar produto', 'Tente novamente');
    if (value >= 99) return;
  ...
  const onPressRemove = (item: prodModel) => {
    let value: number | undefined = shoppingList.has(item.prod_id)
      ...
      : 0;
    if (typeof value == 'undefined')
      return Alert.alert('Erro ao remover produto', 'Tente novamente');
    if (value > 1) {
      setShoppingList(
        shoppingList.set(item.prod_id, { quantity: value - 1, item: item })
      );
...
function updateCart(
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (item) final.add(item.item.price.value * item.quantity);
  }
  return final;
}

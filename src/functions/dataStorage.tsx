...
export async function getUserStatus(): Promise<
  'returning' | 'returning&logged' | undefined
> {
...
export async function getShortAddress() {
  ...
  return (
    (address.rua !== '' ? address.rua : address.cidade) +
...
export async function getNotify() {
  ...
  return new Map<string, prodModel>(
...
export async function saveShoppingList(
  value: Map<string, { quantity: number; item: prodModel }>
) {
  const stringValue = JSON.stringify(Array.from(value.entries()));
...
export async function getShoppingList() {
  ...
  return new Map<string, { quantity: number; item: prodModel }>(
    stringValue ? JSON.parse(stringValue) : null
  );
}
...
export async function saveActiveMarketKey(value: string, city?: string) {
  ...
  fetch(requests + `mercList.php?city=${city}&market=${value}`)
    ...
    .then((json: marketModel[]) => saveActiveMarket(createMercItem(json[0])))
...
// activeMarket
async function saveActiveMarket(value: marketModel) {
  const stringValue = JSON.stringify(value);
  await AsyncStorage.setItem(suffix + 'activeMarket', stringValue);
}

export async function getActiveMarket(): Promise<marketModel> {
...
export async function getLastPayment(): Promise<{
  title: string;
  sub: string;
}> {

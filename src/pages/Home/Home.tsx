...
function HomeHeader({
  ...
  const [shortAdress, setShortAdress] = useState<string>(
    'Carregando endereço...'
  );

  useFocusEffect(
    React.useCallback(() => {
      getShortAddress().then((shortAdress) => {
        if (shortAdress) setShortAdress(shortAdress);
      });
    }, [])
  );
  ...
  return (
    <View
      ...
      <View style={styles.icon}>
        <Icon name='map-marker' size={30} color={myColors.primaryColor} />
        <MyButton
          type='clear'
          title={shortAdress}
          titleStyle={{
